...
export class ProductComponent implements AfterViewInit {
  data: any[] = []; // Nơi lưu trữ dữ liệu từ file Excel

  onFileChange(event: any): void {
    const target: DataTransfer = <DataTransfer>(event.target);
    if (target.files.length !== 1) {
      alert('Vui lòng chọn đúng một file Excel!');
      return;
    }
    ...
    reader.onload = (e: any) => {
      ...
      console.log(this.data); // Hiển thị dữ liệu để kiểm tra
      this.updateSystemData(this.data); // Gửi dữ liệu để cập nhật hệ thống
    };
    reader.readAsBinaryString(target.files[0]);
  }

  updateSystemData(data: any[]): void {
    // Gửi dữ liệu tới API để cập nhật hệ thống
    console.log('Dữ liệu cần cập nhật:', data);
    // Ví dụ: Gửi API
    // this.apiService.updateData(data).subscribe(
    //   response => console.log('Cập nhật thành công:', response),
    //   error => console.error('Cập nhật thất bại:', error)
    // );
  }
